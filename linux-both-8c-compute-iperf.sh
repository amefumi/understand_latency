...
# Perf flamegraph: 0 = disabled, 1 = enabled
PERF_FG=0
...
# Start iperf3: we locate server in remote and client in local
IPERF3_BIN=$TARGETDIR/iperf-3.18/src/iperf3
IPERF_PID=""
# for each core in TASKSET, start iperf3 threads:
BASE_PORT=5201

# --- start two servers on the target, one per core, each on its own port ---
i=0
for core in $(echo $TASKSET | tr "," "\n")
do
	port=$((BASE_PORT + i))
	ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S taskset -c $core nice -n -20 $IPERF3_BIN -s -p $port -1 > /dev/null 2>&1" &
	echo "ssh $USER\@$TARGETC -t \"... taskset -c $core $IPERF3_BIN -s -p $port -1\""
	i=$((i + 1))
done

sleep 3

# --- start two clients locally, one per core, matching ports ---
i=0
for core in $(echo $TASKSET | tr "," "\n")
do
	port=$((BASE_PORT + i))
	# --bidir -P 8 still creates 16 threads, all pinned to this one core
	echo "taskset -c $core $IPERF3_BIN -c $TARGET -p $port --bidir -P $IPERF_CONNS -t $IPERF_TIME -i 10 > $TARGETDIR/latency/temp/iperf3_${core}.log 2>&1 &"
	sudo taskset -c $core nice -n -20 $IPERF3_BIN -c $TARGET -p $port --bidir -P $IPERF_CONNS -t $IPERF_TIME -i 10 > $TARGETDIR/latency/temp/iperf3_${core}.log 2>&1 &
	# sudo taskset -c $core nice -n -20 $IPERF3_BIN -c $TARGET -p $port -P $IPERF_CONNS -t $IPERF_TIME -i 10 > $TARGETDIR/latency/temp/iperf3_${core}.log 2>&1 &
...
ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S killall iperf3"
ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S -v && echo 0 | sudo tee /sys/module/core/parameters/accu_irq_accounting"
ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S -v && echo 0 | sudo tee /sys/module/core/parameters/scheduler_accounting"
