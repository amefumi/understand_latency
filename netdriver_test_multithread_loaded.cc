...
void test_ndping_send(struct sockaddr *dest, int id, int io_depth, int flow_size, int src_port, int busy_wait_us)
{
...
//	sched_setscheduler(pid, SCHED_RR, &param);
	...
	while(1) {
		...
		while(total < flow_size) {
			...
			if(total == flow_size) { // As flow_size is equal to a single packet, this will be triggered once per packet

				// Put busy wait logic here
				if (busy_wait_us > 0) {
				...
				// Alternative busy wait logic that does some computation instead of just spinning
				// if (busy_wait_us > 0) {
...
//	printf("buffer:%s\n", buffer);
	...
	*port_name = 0;
	...
	for (nextArg = 2; (nextArg < argc) && (*argv[nextArg] == '-');
		...
		if (strcmp(argv[nextArg], "--help") == 0) {
		...
		} else if (strcmp(argv[nextArg], "--busy_wait_us") == 0) {
			if (nextArg == (argc-1)) {
				printf("No value provided for %s option\n",
					argv[nextArg]);
				exit(1);
			...
			nextArg++;
			busy_wait_us = get_int(argv[nextArg],
	...
	for(i = 0; i < thread_count; i++) {
		...
		for ( ; nextArg < argc; nextArg++) {
			if (strcmp(argv[nextArg], "tcpppasync") == 0) {
				workers.push_back(std::thread(test_ndping_send, dest, i, io_depth, flow_size, srcPort + i, busy_wait_us));
				if(pin == 1) {
					cpu_set_t cpuset;
					CPU_ZERO(&cpuset);
