...
sudo sysctl -w net.core.latency_dumb_schedule_disable_clamp=1 # 0 means enable clamp now
...
ssh $USER\@$TARGETC -t "
  echo '$SUDOPW' | sudo -S zsh -c '
    ulimit -n 65536
    exec taskset -c $TASKSET nice -n -20 $TARGETDIR/latency/pingpong_server_heter \
...
echo "sudo taskset -c $TASKSET nice -n -20 $TARGETDIR/latency/pingpong_server_heter  --ip $TARGET --port $((DPORT)) --count $N --iodepth $IODEPTH --flowsize $SIZE --pin $PIN  > $TARGETDIR/latency/temp/server.log"
sleep 3
...
sudo zsh -c 'ulimit -n 65536; exec taskset -c '"$TASKSET"' nice -n -20 ./netdriver_test_multithread_heter '"$TARGET"':'"$DPORT"' --count '"$N"'  --iodepth '"$IODEPTH"' --flowsize '"$SIZE"' --pin '"$PIN"' --sc '"$SC"' --time '"$CLIENT_TIME"' tcpppasync > temp/client.log' &

echo "echo $SUDOPW | sudo -S taskset -c $TASKSET nice -n -20 ./netdriver_test_multithread_heter $TARGET:$DPORT --count $N  --iodepth $IODEPTH --flowsize $SIZE --pin $PIN --sc $SC --time $CLIENT_TIME tcpppasync"
PIDS="$PIDS $!"
