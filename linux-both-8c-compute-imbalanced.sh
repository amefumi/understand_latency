...
#	sudo ethtool -C $INTF rx-frames $PKT rx-usecs $TIMEOUT tx-frames $PKT tx-usecs $TIMEOUT
    ...
    USECS=$(((20*N/SC+11)/12)) # usecs = (N/4) * (1/(0.4/2)) = 1.25 * N
...
sudo zsh -c 'ulimit -n 65536; exec taskset -c '"$TASKSET"' nice -n -20 ./netdriver_test_multithread_imbalanced '"$TARGET"':'"$DPORT"' --count '"$N"'  --iodepth '"$IODEPTH"' --flowsize '"$SIZE"' --pin '"$PIN"' --sc '"$SC"' --time '"$CLIENT_TIME"' --inbalancediodepth 256 tcpppasync > temp/client.log' &

echo "echo $SUDOPW | sudo -S taskset -c $TASKSET nice -n -20 ./netdriver_test_multithread_imbalanced $TARGET:$DPORT --count $N  --iodepth $IODEPTH --flowsize $SIZE --pin $PIN --sc $SC --time $CLIENT_TIME tcpppasync"
PIDS="$PIDS $!"
