...
our_patch="1"
c_state=1
# num_apps = [2, 4, 8, 16, 24, 32, 36, 40, 44, 48, 52, 56, 64, 72, 80]
num_apps = [56, 64]
flowsize = [64]
iodepth = [1]
dim = [1] # only apply to server side, client side is always dim=0
pin = [1]
permute = [1] # no need to permute: client side never become bottleneck
hrtick = [0]
sched = [0]
cores = [16]
runs = [0, 1, 2]
...
def main():
    ...
    for n, f, i, d, p, perm, h, s, core, run in combinations:
        ...
        command = f"./linux-both-8c-compute-decouple-client.sh {n} {DIR} {f} {i} {d} {p} {perm} {h} {s} {core} {run} {dim_monitor}"
        print(command)
