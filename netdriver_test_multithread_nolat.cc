...
//	sched_setscheduler(pid, SCHED_RR, &param);
	...
	getcpu(&cpu, &node);
	clock_gettime(CLOCK_REALTIME, &begin_time); // keep begin time for throughput calculation 
	while(burst > 0) {
		total = 0;
	...
	while(1) {
		...
		// time_q.push(end);
		if(stop_count == 1) {
			clock_gettime(CLOCK_REALTIME, &end_time); // we add the end_time record here, to get throughput
			break;
		}
	...
	tfile.open("temp/netperf-" + std::to_string(id)+"_throughput.log");
	tfile << pid << " " << ntohs(client.sin_port) << " " << sent_packets  / (diff_us(begin_time, end_time) / 1000000.0)  << std::endl;
