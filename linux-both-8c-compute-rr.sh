...
#	sudo ethtool -C $INTF rx-frames $PKT rx-usecs $TIMEOUT tx-frames $PKT tx-usecs $TIMEOUT
	...
	ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S ethtool -C $INTF adaptive-rx off adaptive-tx off"
	ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S ethtool -C $INTF rx-frames 64 rx-usecs 64 tx-frames 128 tx-usecs 32"
	sudo ethtool -C $INTF adaptive-rx off adaptive-tx off
	sudo ethtool -C $INTF rx-frames 64 rx-usecs 64 tx-frames 128 tx-usecs 32
fi
...
# # Enable packets processed in softIRQ monitoring
...
# # Perf: periodic calling graph sampling
...
# # Perf: L1d-cache-loads, L1d-cache-load-misses, LLC-loads
# echo $SUDOPW | sudo -S /home/ame/pmu-tools/ocperf.py stat -C 32 -e MEM_INST_RETIRED.ALL_LOADS,L2_RQSTS.ALL_DEMAND_DATA_RD,OCR.DEMAND_DATA_RD.ANY_RESPONSE -- sleep $CLIENT_TIME &> $TARGETDIR/latency/temp/perf_overall_cache_client.log &
# ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S /home/ame/pmu-tools/ocperf.py stat -C 32 -e MEM_INST_RETIRED.ALL_LOADS,L2_RQSTS.ALL_DEMAND_DATA_RD,OCR.DEMAND_DATA_RD.ANY_RESPONSE -- sleep $CLIENT_TIME &>$TARGETDIR/latency/temp/perf_overall_cache_server.log" &
...
ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S taskset -c $TASKSET nice -n -20 $TARGETDIR/latency/pingpong_server_rr  --ip $TARGET --port $((DPORT)) --count $N --iodepth $IODEPTH --flowsize $SIZE --pin $PIN --permute $PERMUTE --sc $SC > $TARGETDIR/latency/temp/server.log" &
echo "sudo taskset -c $TASKSET nice -n -20 $TARGETDIR/latency/pingpong_server_rr  --ip $TARGET --port $((DPORT)) --count $N --iodepth $IODEPTH --flowsize $SIZE --pin $PIN  > $TARGETDIR/latency/temp/server.log"
sleep 3
...
sudo taskset -c $TASKSET nice -n -20 ./netdriver_test_multithread_rr $TARGET:$DPORT --count $N  --iodepth $IODEPTH --flowsize $SIZE --pin $PIN --sc $SC --time $CLIENT_TIME tcpppasync  > temp/client.log &
echo "echo $SUDOPW | sudo -S taskset -c $TASKSET nice -n -20 ./netdriver_test_multithread_rr $TARGET:$DPORT --count $N  --iodepth $IODEPTH --flowsize $SIZE --pin $PIN --sc $SC --time $CLIENT_TIME tcpppasync"
PIDS="$PIDS $!"
