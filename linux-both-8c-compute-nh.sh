...
#	sudo ethtool -C $INTF rx-frames $PKT rx-usecs $TIMEOUT tx-frames $PKT tx-usecs $TIMEOUT
	...
	ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S ethtool -C $INTF adaptive-rx off adaptive-tx off"
	ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S ethtool -C $INTF rx-frames 64 rx-usecs 64 tx-frames 128 tx-usecs 64"
	# ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S ethtool -C $INTF rx-frames 64 rx-usecs 64"
	sudo ethtool -C $INTF adaptive-rx off adaptive-tx off
	sudo ethtool -C $INTF rx-frames 64 rx-usecs 64 tx-frames 128 tx-usecs 64
	# sudo ethtool -C $INTF rx-frames 64 rx-usecs 64
fi
...
ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S taskset -c $TASKSET nice -n -20 $TARGETDIR/latency/pingpong_server_nh  --ip $TARGET --port $((DPORT)) --count $N --iodepth $IODEPTH --flowsize $SIZE --pin $PIN --permute $PERMUTE --sc $SC > $TARGETDIR/latency/temp/server.log" &
echo "sudo taskset -c $TASKSET nice -n -20 $TARGETDIR/latency/pingpong_server_nh  --ip $TARGET --port $((DPORT)) --count $N --iodepth $IODEPTH --flowsize $SIZE --pin $PIN  > $TARGETDIR/latency/temp/server.log"
sleep 3
...
sudo taskset -c $TASKSET nice -n -20 ./netdriver_test_multithread_nh $TARGET:$DPORT --count $N  --iodepth $IODEPTH --flowsize $SIZE --pin $PIN --sc $SC --time $CLIENT_TIME tcpppasync  > temp/client.log &
echo "echo $SUDOPW | sudo -S taskset -c $TASKSET nice -n -20 ./netdriver_test_multithread_nh $TARGET:$DPORT --count $N  --iodepth $IODEPTH --flowsize $SIZE --pin $PIN --sc $SC --time $CLIENT_TIME tcpppasync"
PIDS="$PIDS $!"
...
# sudo tail -n 1000  /var/log/kern.log > temp/filter_client.log
# ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S -v && sudo tail -n 1000  /var/log/kern.log" > temp/filter_server.log
