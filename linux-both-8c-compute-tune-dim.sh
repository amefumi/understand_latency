...
sudo sysctl -w net.core.latency_dumb_schedule_tcp_send=1
...
ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S sysctl -w net.core.latency_dumb_schedule_tcp_send=1"
...
#	sudo ethtool -C $INTF rx-frames $PKT rx-usecs $TIMEOUT tx-frames $PKT tx-usecs $TIMEOUT
    ...
    USECS=$(((5*N+3)/4)) # usecs = (N/4) * (1/(0.4/2)) = 1.25 * N
