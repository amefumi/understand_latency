...
# ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S -v && sudo insmod $TARGETDIR/netfilter/filter.ko"
...
sleep 3 # this is to wait remote to really clean the socket.
...
ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S taskset -c $TASKSET nice -n -20 $TARGETDIR/latency/pingpong_server_loaded  --ip $TARGET --port $((DPORT)) --count $N --iodepth $IODEPTH --flowsize $SIZE --pin $PIN --permute $PERMUTE --sc $SC --busy_wait_us $LOAD > $TARGETDIR/latency/temp/server.log" &
echo "sudo taskset -c $TASKSET nice -n -20 $TARGETDIR/latency/pingpong_server_loaded  --ip $TARGET --port $((DPORT)) --count $N --iodepth $IODEPTH --flowsize $SIZE --pin $PIN --permute $PERMUTE --sc $SC --busy_wait_us $LOAD > $TARGETDIR/latency/temp/server.log"
sleep 3
...
sudo taskset -c $TASKSET nice -n -20  ./netdriver_test_multithread_loaded $TARGET:$DPORT --count $N  --iodepth $IODEPTH --flowsize $SIZE --pin $PIN --sc $SC --busy_wait_us $LOAD tcpppasync > temp/client.log &
echo "sudo taskset -c $TASKSET nice -n -20  ./netdriver_test_multithread_loaded $TARGET:$DPORT --count $N  --iodepth $IODEPTH --flowsize $SIZE --pin $PIN --sc $SC --busy_wait_us $LOAD tcpppasync"
PIDS="$PIDS $!"
echo "pid $PIDS dport $DPORT"

sar -u 55 2 -P ALL > $DIR/cpu-$N.log &
ssh $USER\@$TARGETC -t "sar -u 55 2 -P ALL" > $DIR/cpu-server-$N.log &
...
ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S trace-cmd clear"
ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S killall pingpong_server_loaded"
ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S -v && echo 0 | sudo tee /sys/module/core/parameters/accu_irq_accounting"
...
# ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S -v && sudo tail -n 32  /var/log/kern.log" > temp/filter_server.log
...
# remove iter_thread
sudo tail -n 550  /var/log/kern.log > $DIR/iter_thread_client.log
sudo rmmod iter_thread
ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S tail -n 550  /var/log/kern.log" > $DIR/iter_thread_server.log
