...
// To prevent the compiler from optimizing away the heavy_compute() calls
 ...
 */
void nd_pingpong(int fd, struct sockaddr_in source, int iodepth, int flow_size, int busy_wait_us)
{
...
//	fflush (stdout);
	...
	while (1) {
		...
		// Do a busy wait when receive a packet
		if (busy_wait_us > 0) {
			int target_ns = busy_wait_us * 1000;
			struct timespec busy_wait_start, busy_wait_now;
			clock_gettime(CLOCK_MONOTONIC, &busy_wait_start);
			do {
				clock_gettime(CLOCK_MONOTONIC, &busy_wait_now);
			} while (((busy_wait_now.tv_sec - busy_wait_start.tv_sec) * 1e9 + (busy_wait_now.tv_nsec - busy_wait_start.tv_nsec)) < target_ns);
		}

		// if (busy_wait_us > 0) {
		// 	long iters = 1000L * busy_wait_us;
		// 	struct timespec seed;
		// 	clock_gettime(CLOCK_MONOTONIC, &seed);
		// 	double s = 1.0;
		// 	s += (seed.tv_nsec % 997) * 1e-9;
		// 	for (long i = 1; i <= iters; ++i) {
		// 		s += sqrt((double)i * s) * 1.0000001;
		// 	}
		// 	sink = s;
		// }

		// End of busy wait logic
 ...
 */
void tcp_server(int port, int num_threads, int iodepth, int flow_size, int pin, int permute, int sc, int busy_wait_us)
{
	...
	while (1) {
		...
		if(not_created && conns == num_threads) {
			for (i = 0; i < num_threads; i++) {
				...
				data = socklist.front();
				socklist.pop_front();
				std::thread thread(nd_pingpong, data.fd, data.source, data.iodepth, data.flow_size, busy_wait_us);
				if(pin == 1) {
					cpu_set_t cpuset;
					CPU_ZERO(&cpuset);
...
int main(int argc, char** argv) {
	...
	for (next_arg = 1; next_arg < argc; next_arg++) {
		if (strcmp(argv[next_arg], "--help") == 0) {
		...
		} else if (strcmp(argv[next_arg], "--busy_wait_us") == 0) {
			if (next_arg == (argc-1)) {
				printf("No value provided for %s option\n",
					argv[next_arg]);
				exit(1);
			...
			next_arg++;
			busy_wait_us = get_int(argv[next_arg],
				"Bad busy wait time %s; must be positive integer\n");
		} else {
	...
	// }
	workers.push_back(std::thread(tcp_server, port, thread_count, iodepth, flow_size, pin, permute, sc, busy_wait_us));
