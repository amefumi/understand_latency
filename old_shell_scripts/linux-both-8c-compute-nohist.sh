...
sudo taskset -c $TASKSET nice -n -20  ./netdriver_test_multithread_nohist $TARGET:$DPORT --count $N  --iodepth $IODEPTH --flowsize $SIZE --pin $PIN --sc $SC tcpppasync  > temp/client.log &
echo "sudo taskset -c $TASKSET nice -n -20  ./netdriver_test_multithread_nohist $TARGET:$DPORT --count $N  --iodepth $IODEPTH --flowsize $SIZE --pin $PIN --sc $SC tcpppasync"
PIDS="$PIDS $!"
