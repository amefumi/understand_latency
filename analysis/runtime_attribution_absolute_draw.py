...
if __name__ == "__main__":
    ...
    for idx, thread in enumerate(threads_info):
        if thread.client_core == 32:
            ...
            ratio.append((thread.client_abs_vruntime - combined_value) / thread.client_abs_vruntime)

    if subtracted:
        min_residual = min(client_residuals)
        combined_metrics = [v + min_residual for v in combined_metrics]
    
    print("Client residuals ratio:", ratio)
    ...
    ax.plot(np.arange(len(combined_metrics)), combined_metrics, label="Modeled:   Client", marker=markers[2], zorder=3, linestyle=(0, (1, 1)), color="darkturquoise", markersize=4, solid_capstyle="round", markeredgewidth=2)
    ...
    for idx, thread in enumerate(threads_info):
        if thread.client_core == 32:
            ...
            ratio.append((thread.server_abs_vruntime - combined_value) / thread.server_abs_vruntime)

    if subtracted:
        min_residual = min(server_residuals)
        combined_metrics = [v + min_residual for v in combined_metrics]

    print("Server residuals ratio:", ratio)
    ...
    ax.plot(np.arange(len(combined_metrics)), combined_metrics, label="Server", marker=markers[3], zorder=5, linestyle=(0, (1, 1)), color="cornflowerblue", markersize=4, solid_capstyle="round", markeredgewidth=1.5)

    ax.set_yticks([5.5e7, 6e7, 6.5e7, 7e7, 7.5e7])#, ["5.5x10$^7$", "6x10$^7$", "6.5x10$^7$", "7x10$^7$", "7.5x10$^7$"])
    ax.set_ylim(5.5e7-1e6, 7.5e7)
    ...
    fig.savefig(os.path.join(result_dir, f"runtime_attribution_absolute_260120.pdf"), bbox_inches=bbox)
    plt.close(fig)
