...
if __name__ == "__main__":
    ...
    for index, experiment, n_thread, label in zip(range(len(experiments)), experiments, n_threads, labels):
        ...
        for t in range(len(threads_info[0].server_vruntime)):
            vruntime_clients = [thread.client_vruntime[t] for thread in threads_info if thread.client_core == 96]
            vruntime_servers = [thread.server_vruntime[t] for thread in threads_info if thread.server_core == 96]
        ...
        max_client_vruntime_last = 4e6
        max_server_vruntime_last = 4e6
