    ...
    for port, port_samples in samples.items():
        ...
        for idx, ts in enumerate(port_samples):
            if is_valid(ts['valid'], stage_bits['rx_data_copy']):
                ...
                accounted_latency[port]['rxc_dram'].append(ts['rxc_dram'])
            ...
            if is_valid(ts['valid'], stage_bits['app']):
                ...
                accounted_latency[port]['app_dram'].append(ts['app_dram'])
            ...
            if is_valid(ts['valid'], stage_bits['tx_data_copy']):
                ...
                accounted_latency[port]['txc_dram'].append(ts['txc_dram'])
...
def process_experiment(result_dir, experiment, n_thread, save_id):
    ...
    for port in client_accounted.keys():
        ...
        client_app_dram = [x for x in client_accounted[port]['app_dram']]
        ...
        client_rxc_dram = [x for x in client_accounted[port]['rxc_dram']]
        ...
        client_txc_dram = [x for x in client_accounted[port]['txc_dram']]
        ...
        client_results[-1]['app_dram'] = sum(client_app_dram)
        ...
        client_results[-1]['rxc_dram'] = sum(client_rxc_dram)
        ...
        client_results[-1]['txc_dram'] = sum(client_txc_dram)
    ...
    for port in server_accounted.keys():
        ...
        server_app_dram = [x for x in server_accounted[port]['app_dram']]
        ...
        server_rxc_dram = [x for x in server_accounted[port]['rxc_dram']]
        ...
        server_txc_dram = [x for x in server_accounted[port]['txc_dram']]
        ...
        server_results[-1]['app_dram'] = sum(server_app_dram)
        ...
        server_results[-1]['rxc_dram'] = sum(server_rxc_dram)
        ...
        server_results[-1]['txc_dram'] = sum(server_txc_dram)
    ...
    with open(client_save_path, "w") as f:
        f.write("port rxc_avg rxc_count rxc_l1 rxc_l2 rxc_l3 rxc_dram app_avg app_count app_l1 app_l2 app_l3 app_dram txc_avg txc_count txc_l1 txc_l2 txc_l3 txc_dram\n")
        for res in sorted(client_results, key=lambda r: r['port']):
            f.write(f"{res['port']} {res['rxc_avg']} {res['rxc_count']} {res['rxc_l1']} {res['rxc_l2']} {res['rxc_l3']} {res['rxc_dram']} "
                    f"{res['app_avg']} {res['app_count']} {res['app_l1']} {res['app_l2']} {res['app_l3']} {res['app_dram']} "
                    f"{res['txc_avg']} {res['txc_count']} {res['txc_l1']} {res['txc_l2']} {res['txc_l3']} {res['txc_dram']}\n")
