...
def parse_experiment(experiment_name):
    ...
    for n, f, i, d, p, perm, h, s, core in combinations:
        ...
        for run in runs:
            ...
            client_total_interrupts += sum(client_diff[:32]) # NOTE: Qizhe uses average for top 2 interrupts, but I just use sum.
            server_total_interrupts += sum(server_diff[:32])
...
def parse_by_path(experiment_dir):
    ...
    client_total_interrupts += sum(client_diff[:32])/16 # NOTE: Qizhe uses average for top 2 interrupts, but I just use sum.
    server_total_interrupts += sum(server_diff[:32])/16
...
if __name__ == "__main__":
    ...
    experiments = ["sirq_pcbs_breakdown_cores_1_1"]
    ...
    # path = "/data0/projects/latency/sirq_pcbs_breakdown_cores_1_1/48_64_1_1_1_1_0_0_16_0"
    # parse_by_path(path)