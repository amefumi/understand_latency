...
if __name__ == "__main__":
    ...
    out_dir = "/data0/projects/latency/rdpmc_nosmt_stall2_performance"
    ...
    for exp_index, experiment in enumerate(experiments):
        ...
        plot_model(stall_server, time_server, raw_path_server, markers[0], "Average Stall Cycles", "Processing Time (ns)")
        
        raw_path_server_stall = os.path.join(
            out_dir, f"{save_idx[exp_index]}_server_l1d_vs_stall.pdf")
        plot_model(stall_server, stall_l1d_server, raw_path_server_stall, markers[0], "Average Stall Cycles", "Average L1d Miss-caused Stall Cycles")
        ...
        raw_path_client = os.path.join(
            out_dir, f"{save_idx[exp_index]}_client_proc_vs_stall.pdf")
        plot_model(stall_client, time_client, raw_path_client, markers[1], "Average Stall Cycles", "Processing Time (ns)")
        raw_path_client_stall = os.path.join(
            out_dir, f"{save_idx[exp_index]}_client_l1d_vs_stall.pdf")
        plot_model(stall_client, stall_l1d_client, raw_path_client_stall, markers[1], "Average Stall Cycles", "Average L1d Miss-caused Stall Cycles")
