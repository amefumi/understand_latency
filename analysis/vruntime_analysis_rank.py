...
if __name__ == "__main__":
    ...
    labels = [
        ...
        "sirq_vruntime_2_48_32_5_core32",
        "sirq_vruntime_2_48_32_6_core32",
        "sirq_vruntime_2_48_32_7_core32",
        "sirq_vruntime_2_48_32_8_core32",
        "sirq_vruntime_2_48_32_9_core32",
        # "nirq_pktirq_vruntime_48_1_3",
        # "nirq_pktirq_vruntime_48_1_4",
        # "sirq_understanding_24_48_1_3",
    ...
    for index, experiment, n_thread, label in zip(range(len(experiments)), experiments, n_threads, labels):
        ...
        plt.scatter(clients_avg_ranks, clients_sched, marker='o', label='Client')
        plt.scatter(servers_avg_ranks, servers_sched, marker='s', label='Server')
        plt.xlabel('Average scheduling rank')
        plt.ylabel('P99.9 rx_sched (us)')
        plt.xlim(0, 25)
        plt.ylim(0, 2e7)
        plt.title(f'Experiment: {label}')
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(os.path.join(f"avg_rank_vs_latency_{label}.pdf"))
        plt.close()