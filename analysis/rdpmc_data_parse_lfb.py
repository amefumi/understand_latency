...
def server_sched_parser(breakdown_log_path):
    ...
    for line in lines:
        ...
        if m is not None:
            ...
            if len(timestamps) == 47:
                ...
                p0, p1, p2, p3, p4, p5, p6, p7, p8, p9, p10, p11 = timestamps
    ...
    for port, port_samples in samples.items():
        ...
        for idx, ts in enumerate(port_samples[:-10]):
            if ts['rxc_l3'] > 1000 or ts['app_l3'] > 1000 or ts['txc_l3'] > 1000:
                continue # we skip those outliers in server side
            ...
            if is_valid(ts['valid'], stage_bits['rx_data_copy']):
                ...
                accounted_latency[port]['rxc_lfb_occ'].append(ts['rxc_lfb_occ'])
                accounted_latency[port]['rxc_lfb_cyc'].append(ts['rxc_lfb_cyc'])
            ...
            if is_valid(ts['valid'], stage_bits['app']):
                ...
                accounted_latency[port]['app_lfb_occ'].append(ts['app_lfb_occ'])
                accounted_latency[port]['app_lfb_cyc'].append(ts['app_lfb_cyc'])
            ...
            if is_valid(ts['valid'], stage_bits['tx_data_copy']):
                ...
                accounted_latency[port]['txc_lfb_occ'].append(ts['txc_lfb_occ'])
                accounted_latency[port]['txc_lfb_cyc'].append(ts['txc_lfb_cyc'])
...
def client_sched_parser(breakdown_log_path):
    ...
    pattern = re.compile(
        ...
        r"p5: ([0-9]+) p6: ([0-9]+) p7: ([0-9]+) p8: ([0-9]+) p9: ([0-9]+) "
        r"p10: ([0-9]+) p11: ([0-9]+).*"
    )
    ...
    for line in lines:
        ...
        if m is not None:
            ...
            if len(timestamps) == 47:
                ...
                if not count[port] < 1000: # We skip first 100 samples
                    samples[port].append({
                        'rx_read_enter': rx_read,
                        'rx_sleep_enter': rx_sleep,
                        'rx_wake_up': rx_wake_up,
...
def process_experiment(result_dir, experiment, n_thread, save_id):
    ...
    for port in client_accounted.keys():
        ...
        client_app_lfb_occ = [x for x in client_accounted[port]['app_lfb_occ']]
        client_app_lfb_cyc = [x for x in client_accounted[port]['app_lfb_cyc']]
        ...
        client_rxc_lfb_occ = [x for x in client_accounted[port]['rxc_lfb_occ']]
        client_rxc_lfb_cyc = [x for x in client_accounted[port]['rxc_lfb_cyc']]
        ...
        client_txc_lfb_occ = [x for x in client_accounted[port]['txc_lfb_occ']]
        client_txc_lfb_cyc = [x for x in client_accounted[port]['txc_lfb_cyc']]
        ...
        client_results[-1]['app_lfb_occ'] = sum(client_txc_lfb_occ)
        client_results[-1]['app_lfb_cyc'] = sum(client_txc_lfb_cyc)
        ...
        client_results[-1]['rxc_lfb_occ'] = sum(client_rxc_lfb_occ)
        client_results[-1]['rxc_lfb_cyc'] = sum(client_rxc_lfb_cyc)
        ...
        client_results[-1]['txc_lfb_occ'] = sum(client_txc_lfb_occ)
        client_results[-1]['txc_lfb_cyc'] = sum(client_txc_lfb_cyc)
    ...
    for port in server_accounted.keys():
        ...
        server_app_lfb_occ = [x for x in server_accounted[port]['app_lfb_occ']]
        server_app_lfb_cyc = [x for x in server_accounted[port]['app_lfb_cyc']]
        ...
        server_rxc_lfb_occ = [x for x in server_accounted[port]['rxc_lfb_occ']]
        server_rxc_lfb_cyc = [x for x in server_accounted[port]['rxc_lfb_cyc']]
        ...
        server_txc_lfb_occ = [x for x in server_accounted[port]['txc_lfb_occ']]
        server_txc_lfb_cyc = [x for x in server_accounted[port]['txc_lfb_cyc']]
        ...
        server_results[-1]['app_lfb_occ'] = sum(server_app_lfb_occ)
        server_results[-1]['app_lfb_cyc'] = sum(server_app_lfb_cyc)
        ...
        server_results[-1]['rxc_lfb_occ'] = sum(server_rxc_lfb_occ)
        server_results[-1]['rxc_lfb_cyc'] = sum(server_rxc_lfb_cyc)
        ...
        server_results[-1]['txc_lfb_occ'] = sum(server_txc_lfb_occ)
        server_results[-1]['txc_lfb_cyc'] = sum(server_txc_lfb_cyc)


    # Save results
    client_save_path = os.path.join(experiment_dir, f"rdpmc_client_4.txt")
    server_save_path = os.path.join(experiment_dir, f"rdpmc_server_4.txt")
    with open(client_save_path, "w") as f:
        f.write("port rxc_avg rxc_count rxc_l2 rxc_l3 rxc_lfb_occ rxc_lfb_cyc app_avg app_count app_l2 app_l3 app_lfb_occ app_lfb_cyc txc_avg txc_count txc_l2 txc_l3 txc_lfb_occ txc_lfb_cyc\n")
        for res in sorted(client_results, key=lambda r: r['port']):
            f.write(f"{res['port']} {res['rxc_avg']:.3f} {res['rxc_count']} {res['rxc_l2']} {res['rxc_l3']} {res['rxc_lfb_occ']} {res['rxc_lfb_cyc']} "
                    f"{res['app_avg']:.3f} {res['app_count']} {res['app_l2']} {res['app_l3']} {res['app_lfb_occ']} {res['app_lfb_cyc']} "
                    f"{res['txc_avg']:.3f} {res['txc_count']} {res['txc_l2']} {res['txc_l3']} {res['txc_lfb_occ']} {res['txc_lfb_cyc']}\n")
...
if __name__ == "__main__":
    ...
    experiments = [
        "sirq_rdpmc_lfb_nofence_light_2/48_64_1_1_1_1_0_0_1_0",
        "sirq_rdpmc_lfb_nofence_light_2/48_64_1_1_1_1_0_0_1_1",
        "sirq_rdpmc_lfb_nofence_light_2/48_64_1_1_1_1_0_0_1_2",
        "sirq_rdpmc_lfb_nofence_light_2/48_64_1_1_1_1_0_0_1_3",
        "sirq_rdpmc_lfb_nofence_light_2/48_64_1_1_1_1_0_0_1_4",
        "sirq_rdpmc_lfb_nofence_light_2/48_64_1_1_1_1_0_0_1_5",
        "sirq_rdpmc_lfb_nofence_light_2/48_64_1_1_1_1_0_0_1_6",
        "sirq_rdpmc_lfb_nofence_light_2/48_64_1_1_1_1_0_0_1_7",
        "sirq_rdpmc_lfb_nofence_light_2/48_64_1_1_1_1_0_0_1_8",
        "sirq_rdpmc_lfb_nofence_light_2/48_64_1_1_1_1_0_0_1_9"
    ]
    n_threads = [
