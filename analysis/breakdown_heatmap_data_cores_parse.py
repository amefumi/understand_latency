...
def breakdown_heatmap(runs, dim, n_thread, result_dir):
    combined_latencies = []
    for run in runs:
        breakdown_file_client = os.path.join(result_dir, f"{n_thread}_64_1_{dim}_1_1_0_0_16_{run}", f"latencies-{n_thread*16}.log")
        breakdown_file_server = os.path.join(result_dir, f"{n_thread}_64_1_{dim}_1_1_0_0_16_{run}", f"latencies-{n_thread*16}-server.log")
...
if __name__ == "__main__":
    ...
    experiments = ["sirq_pcbs_breakdown_cores_newstart_1_1"]
    threads = [64]
    runs = [0, 1, 2]
    dim = 1
