...
colors = ["#bf1a15", "#dd883f", "#497ade", "#639d4e", "#8c17f6", "#b77294"]
...
markers_size = [5, 5, 5, 5, 5]
markers_width = [2, 2, 2, 2, 2]
marker_facecolor = ["none", "none", "none", "none", "none"]
zorder = [6, 7, 8, 9, 7]
result_dir = "/data0/projects/latency/"
saved_file_name = "latency_throughput_apache_wrk2_dimoff"
saved_file_name_dimon = "latency_throughput_apache_wrk2"
# saved_file_name = "latency_throughput_dimon"
saved_figure_name = "latency_throughput_curve_apache_helloworld_wrk2"
# experiments = ["combined_nirq", "combined_dirq", "combined_sirq", "combined_pcbs"] #, "combined_autodim"]
experiments = ["combined_nirq", "combined_sirq"]

# labels = ["Linux", "Linux + IRQa", "Linux + ACCa", "Linux + PCSched", "Linux + PCSched + AutoDIM"]
...
# ax.annotate('DIM off', xy=(35, max_latency * 0.4), horizontalalignment='center', fontsize=14, color='black', zorder=4,
#             bbox=dict(
#                 # facecolor='#e8e8e8',
#                 facecolor='oldlace',
#                 edgecolor='none',
...
# ax.annotate('DIM on', xy=(85, max_latency * 0.4), horizontalalignment='center', fontsize=14, color='black', zorder=3,
#             path_effects=[
#                 path_effects.Stroke(linewidth=0.1, foreground='black'),
#                 path_effects.Normal()
#             ])
...
for experiment_index, experiment_name in enumerate(experiments):
    ...
    num_threads, avg_lat999, avg_throughput = data[:, 0], data[:, 1]*1000, data[:, 2]/1000
    # Draw the latency-throughput curve for each series
    ax.plot(avg_throughput, avg_lat999, color=colors[experiment_index], label=labels[experiment_index]+" (DIM off)", zorder=zorder[experiment_index], alpha=0.5,
...
for experiment_index, experiment_name in enumerate(experiments):
    # Read the data file
    latency_throughput_data = os.path.join(result_dir, experiment_name, f"{saved_file_name_dimon}.data")
    assert os.path.exists(latency_throughput_data), f"Data file {latency_throughput_data} does not exist."
    data = np.loadtxt(latency_throughput_data, delimiter=' ')
    num_threads, avg_lat999, avg_throughput = data[:, 0], data[:, 1]*1000, data[:, 2]/1000
    # Draw the latency-throughput curve for each series
    ax.plot(avg_throughput, avg_lat999, color=colors[experiment_index], label=labels[experiment_index]+" (DIM on)", zorder=zorder[experiment_index],
             marker=markers[experiment_index], markersize=markers_size[experiment_index], markerfacecolor=marker_facecolor[experiment_index], markeredgewidth=markers_width[experiment_index])
    # Optional: annotate each point with the number of threads
    # for i, txt in enumerate(num_threads):
        # ax.annotate(int(txt), (avg_throughput[i], avg_lat999[i]), textcoords="offset points", xytext=(4,4), ha='center', fontsize=6, color=colors[experiment_index], 
...
ax.set_xlabel('Throughput (kIOPS)', labelpad=8)
ax.set_ylabel('P99 Latency (us)', labelpad=8)

ax.set_xlim(0, max_throughput)
ax.set_xticks(np.arange(0, max_throughput+20, 20), ["0", "20", "40", "60", "80", "100"])
