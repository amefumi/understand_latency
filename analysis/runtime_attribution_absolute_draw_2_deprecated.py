...
if __name__ == "__main__":
    ...
    experiment = "sirq_understanding_24_1/48_64_1_1_1_1_0_0_1_6"
    n_thread = 48
    ...
    fig_client = plt.figure(figsize=(fig_width, fig_height))
    fig_server = plt.figure(figsize=(fig_width, fig_height))
    ax_width_frac = ax_width / fig_width
    ax_height_frac = ax_height / fig_height
    
    ax = fig_client.add_axes([0.3, 0.3, ax_width_frac, ax_height_frac])
    ax_server = fig_server.add_axes([0.3, 0.3, ax_width_frac, ax_height_frac])

    


    ax.set_yticks([5.5e7, 6e7, 6.5e7, 7e7], ["5.5x10$^7$", "6x10$^7$", "6.5x10$^7$", "7x10$^7$"])
    ax.set_ylim(5.5e7, 7e7)
    ax_server.set_yticks([5.5e7, 6e7, 6.5e7, 7e7], ["5.5x10$^7$", "6x10$^7$", "6.5x10$^7$", "7x10$^7$"])
    ax_server.set_ylim(5.5e7, 7e7)
    ...
    k = 1024/88761  # sched_prio_to_weight[0] / sched_prio_to_weight[20]
    ...
    for idx, thread in enumerate(threads_info):
        if thread.client_core == 32:
            ...
            combined_value = thread.packets * thread.client_inflation * k
    ...
    bar_wdith = 0.3
    ax.bar(np.arange(len(client_abs_vruntimes)) - bar_wdith/2, client_abs_vruntimes, width=bar_wdith, label="Virtual runtime, Client", edgecolor="forestgreen", hatch='\\\\\\', fill=False, zorder=3, linewidth=2)
    ax.bar(np.arange(len(combined_metrics)) + bar_wdith/2, combined_metrics, width=bar_wdith, label="Modeled runtime, Client", color="lightcoral", zorder=2, edgecolor="firebrick", hatch='///', fill=False, linewidth=2)
    ...
    for idx, thread in enumerate(threads_info):
        if thread.client_core == 32:
            ...
            combined_value = thread.packets * thread.server_inflation * k
            combined_metrics.append(combined_value)
    ...
    ax_server.bar(np.arange(len(server_abs_vruntimes)) - bar_wdith/2, server_abs_vruntimes, width=bar_wdith, label="Virtual runtime, Server", edgecolor="forestgreen", hatch='\\\\\\', fill=False, zorder=3, linewidth=2)
    ax_server.bar(np.arange(len(combined_metrics)) + bar_wdith/2, combined_metrics, width=bar_wdith, label="Modeled runtime, Server", color="lightskyblue", zorder=2, edgecolor="firebrick", hatch='///', fill=False, linewidth=2)

    ax.set_xlabel("Thread Index")
    ax.set_ylabel("Scaled runtime (ns)")
    ax.legend(loc='upper left', markerfirst=False, labelspacing=0.2, facecolor='white', edgecolor='white', framealpha=1)
    
    ax_server.set_xlabel("Thread Index")
    ax_server.set_ylabel("Scaled runtime (ns)")
    ax_server.legend(loc='upper left', markerfirst=False, labelspacing=0.2, facecolor='white', edgecolor='white', framealpha=1)
    
    fig_client.savefig(os.path.join(result_dir, f"runtime_attribution_absolute_260111.pdf"), dpi=300, bbox_inches='tight')
    plt.close(fig_client)
    fig_server.savefig(os.path.join(result_dir, f"runtime_attribution_absolute_server_260111.pdf"), dpi=300, bbox_inches='tight')
