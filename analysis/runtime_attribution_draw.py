...
if __name__ == "__main__":
    ...
    # experiment = "sirq_understanding_24_1/48_64_1_1_1_1_0_0_1_4"
    ...
    ax.set_xticks(np.arange(0, 3e6+1, 1e6), ["0", "1e6", "2e6", "3e6"])
    ax.set_yticks(np.arange(0, 3e6+1, 1e6), ["0", "1e6", "2e6", "3e6"])
    ax.tick_params(which="both", top=True, right=True)
    ax.set_xlim(-2e5, max_vruntime)
    ax.set_ylim(-2e5, max_vruntime)
    ...
    ax.scatter(client_normed_combined_metrics, client_normed_abs_vruntimes, label="Client", marker="x", zorder=2)
    ...
    ax.scatter(server_normed_combined_metrics, server_normed_abs_vruntimes, label="Server", marker="o", zorder=1)

    ax.set_xlabel("Relative modeled runtime")
    ax.set_ylabel("Relative virtual runtime")
    ax.legend(loc='upper left', markerfirst=False, labelspacing=0.2, facecolor='white', edgecolor='white', framealpha=1)
    fig.savefig(os.path.join(result_dir, f"runtime_attribution_260108.pdf"), dpi=300, bbox_inches='tight')
    plt.close(fig)
