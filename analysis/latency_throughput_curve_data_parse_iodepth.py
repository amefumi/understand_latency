...
experiments = ["nirq_eevdf_curve_iodepth_1_1"]
saved_file_name = "latency_throughput_dimon_20"

num_apps = [20]
flowsize = [64]
iodepth = [1, 2, 4, 8, 16, 24, 36, 48, 64, 80, 96, 112, 128]
dim = [1]
pin = [1]
permute = [1]
hrtick = [0]
...
def parse_experiment(experiment_name):
    ...
    experiment_data = np.zeros((len(iodepth), 3))
    data_index = 0
    ...
    for n, f, i, d, p, perm, h, s, core in combinations:
        ...
        experiment_data[data_index, :] = [i, latency999, throughput]
        data_index += 1
        print(f"Experiment {experiment_name}, io={i}, latency p999: {latency999}, throughput: {throughput}")
