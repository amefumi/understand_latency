...
def parse_all_thread_info(experiment_dir, total_threads):
    ...
    client_inflation_path = os.path.join(experiment_dir, f"valid_breakdown_sum_p90_client.txt")
    with open(client_inflation_path, "r") as client_inflation_log:
        lines = client_inflation_log.readlines()
        for line in lines[1:]:
            items = line.split()
            port, hidden, inflation = int(items[0]), float(items[-2]), float(items[-1])
            for thread in threads_info:
                if thread.port == port:
                    thread.client_inflation = inflation #- hidden

    # read server inflation info
    server_inflation_path = os.path.join(experiment_dir, f"valid_breakdown_sum_p90_server.txt")
    with open(server_inflation_path, "r") as server_inflation_log:
        ...
        for line in lines[1:]:
            ...
            for thread in threads_info:
                if thread.port == port:
                    thread.server_inflation = inflation #- hidden
...
if __name__ == "__main__":
    ...
    experiments = [       
        ...
        "sirq_understanding_9_1/48_64_1_1_1_1_0_0_1_0",
    ]
    ...
    for index, experiment, n_thread, label in zip(range(len(experiments)), experiments, n_threads, labels):
        ...
        ax_vruntime_bar = fig_vruntime_infl_thpt.add_axes([0.1, 0.2, 0.55, 0.6])
        ax_softirq_line = ax_vruntime_bar.twinx()
        ax_residual = fig_vruntime_infl_thpt.add_axes([0.75, 0.2, 0.2, 0.6])
        client_abs_vruntimes = []
        ...
        ax_softirq_line.plot(thread_indices, client_softirq_packets, label="Client Softirq Packets", color="purple", marker="o")
        ax_softirq_line.set_ylabel("Softirq Packets")
        ax_softirq_line.legend(loc="upper right")
        ax_residual.scatter(client_softirq_packets, client_residuals, label="Residual", color="black", marker="x")
        ax_residual.set_xlabel("# Softirq Packets")
        ...
        ax_softirq_line.plot(thread_indices, server_softirq_packets, label="Server Softirq Packets", color="purple", marker="o")
        ax_softirq_line.set_ylabel("Softirq Packets")
        ax_softirq_line.legend(loc="upper right")
