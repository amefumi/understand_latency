...
def server_sched_parser(breakdown_log_path):
    ...
    pattern = re.compile(r".*source port: ([0-9]+) destination port: ([0-9]+) -- rx -- hw: ([0-9]+) alloc: ([0-9]+) irq: ([0-9]+) napi: ([0-9]+) gro: ([0-9]+) ip: ([0-9]+) tcp: ([0-9]+) read: ([0-9]+) sleep: ([0-9]+) ready: ([0-9]+) wakeup: ([0-9]+) data copy: ([0-9]+) return: ([0-9]+) -- tx -- alloc: ([0-9]+) write: ([0-9]+) data copy: ([0-9]+) tcp: ([0-9]+) ip: ([0-9]+) queue: ([0-9]+) xmit: ([0-9]+) finish: ([0-9]+) -- valid: ([0-9]+).*")
    ...
    for line in lines:
        ...
        if m is not None:
            ...
            if len(timestamps) == 24:
                _, port, rx_hw, rx_alloc, rx_irq, rx_napi, rx_gro, rx_ip, rx_tcp, rx_read, rx_sleep, rx_ready, rx_wake_up, rx_data_copy, rx_return, tx_alloc, tx_write, tx_data_copy, tx_tcp, tx_ip, tx_queue, tx_xmit, tx_finish, valid = timestamps
    ...
    for port, port_samples in samples.items():
        ...
        for idx, ts in enumerate(port_samples):
            ...
            if idx % 2 == 0:
                ...
                if is_valid(ts['valid'], stage_bits['app_hidden']) and ts['rx_sleep_enter'] > last_xmit:
                    accounted_latency[port]['app_hidden'].append(ts['rx_sleep_enter'] - last_xmit)
...
def client_sched_parser(breakdown_log_path):
    ...
    for line in lines:
        ...
        if m is not None:
            timestamps = list(map(int, m.groups()))
            if len(timestamps) == 24:
                port, _, rx_hw, rx_alloc, rx_irq, rx_napi, rx_gro, rx_ip, rx_tcp, rx_read, rx_sleep, rx_ready, rx_wake_up, rx_data_copy, rx_return, tx_alloc, tx_write, tx_data_copy, tx_tcp, tx_ip, tx_queue, tx_xmit, tx_finish, valid = timestamps
...
def process_experiment(result_dir, experiment, n_thread, save_id):
    ...
    for port in client_accounted.keys():
        ...
        for serie in series:
            ...
            client_data.sort()
            client_data = client_data[:int(len(client_data))]
            #  if x <= client_threshold[serie]]
    ...
    for port in server_accounted.keys():
        ...
        for serie in series:
            ...
            server_data.sort()
            server_data = server_data[:int(len(server_data))]
            # if x <= server_threshold[serie]]
    ...
    print([x['total_avg'] for x in client_results])
    print([x['total_avg'] for x in server_results])
    

    # Save results
    client_save_path = os.path.join(experiment_dir, f"valid_breakdown_sum_p90_client.txt")
    server_save_path = os.path.join(experiment_dir, f"valid_breakdown_sum_p90_server.txt")
