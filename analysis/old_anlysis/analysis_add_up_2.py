...
def parse_all_thread_info(experiment_dir, total_threads):
    ...
    with open(client_inflation_path, "r") as client_inflation_log:
        ...
        for line in lines[1:]:
            ...
            if len(items) < 5:
                continue
            ports.append(int(items[0]))
            avg_800ns.append(float(items[3]))
        avg_800ns = norm_vruntime(avg_800ns)
        for thread in threads_info:
            if thread.port in ports:
                index = ports.index(thread.port)
                thread.client_inflation = avg_800ns[index]
    ...
    with open(server_inflation_path, "r") as server_inflation_log:  
        ...
        for thread in threads_info:
            if thread.port in ports:
                index = ports.index(thread.port)
                thread.server_inflation = avg_800ns[index]
...
if __name__ == "__main__":
    ...
    experiments = [       
        # "airq_add_up_2_1/52_64_1_1_1_1_0_0_1_2",
    ...
    for index, experiment, n_thread in zip(range(len(experiments)), experiments, n_threads):
        ...
        # ax_vruntime.legend()
        # fig_vruntime.savefig("add_up_abs_vruntime_vs_throughput.pdf", dpi=300, bbox_inches='tight')
        # plt.close(fig_vruntime)
        ...
        #         throughputs.append(thread.thpt)
        #         packets.append(thread.packets)
        #         inflations.append(thread.server_inflation)
        # min_thpt = min(throughputs)
        # abs_server_vruntime_normed = ([v/thpt*min_thpt for v, thpt in zip(abs_server_vruntime_vcsw, throughputs)])
        # abs_server_vruntime_deflated = ([v-(inflation*n_pkt/86) for v, inflation, n_pkt in zip(abs_server_vruntime_vcsw, inflations, packets)])
        ...
        # ax_vruntime_server.scatter(throughputs, abs_server_vruntime_deflated, label="Server (-ivcsw+vcsw, deflated)", color="purple", marker="o")
        ...
        # ax_vruntime_server.legend()
        # # ax_vruntime_server.set_ylim(bottom=6.5e7)
        # fig_vruntime_server.savefig("add_up_abs_vruntime_server_adjusted_vs_throughput_2_sched.pdf", dpi=300, bbox_inches='tight')
        # plt.close(fig_vruntime_server)
        ...
        min_thpt = min(throughputs)
        client_abs_vruntime_deflated = ([v-inflation*n_pkt/86 for v, inflation, n_pkt in zip(client_abs_vruntime_adjusted, inflations, packets)])
        client_abs_vruntime_normed = ([v/thpt*min_thpt for v, thpt in zip(client_abs_vruntime_adjusted, throughputs)])
        ax_vruntime_client.scatter(throughputs, client_abs_vruntime_deflated, label="Client vruntime (-ivcsw, deflated)", color="orange", marker="s")
        ...
        fig_vruntime_client.savefig("add_up_abs_vruntime_client_adjusted_vs_throughput_2_sched.pdf", dpi=300, bbox_inches='tight')
        plt.close(fig_vruntime_client)
        ...
        # for thread in threads_info:
        #     if thread.client_core == 96:
        #         X_client.append([thread.thpt, thread.client_nivcsw, thread.client_nvcsw])
        #         X_server.append([thread.thpt, thread.server_nivcsw, thread.server_nvcsw])
        #         Y_client.append(thread.client_abs_vruntime)
        #         Y_server.append(thread.server_abs_vruntime)
        ...
        # model_server.fit(X_server, Y_server)
        ...
        # print("Server Linear Regression Coefficients:")
        # print(f"  Intercept: {model_server.intercept_}")
        # print(f"  Coefficients: {model_server.coef_}")
