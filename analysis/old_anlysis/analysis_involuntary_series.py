...
class ThreadData:
    ...
    def __str__(self):
        return (f"Port: {self.port}, Througput: {self.thpt}, Latency: {self.latency}, "
                ...
                f"Client NIVCSW: {self.client_nivcsw}, Client Abs Vruntime: {self.client_abs_vruntime}, "
                ...
                f"Server NIVCSW: {self.server_nivcsw}, Server Abs Vruntime: {self.server_abs_vruntime}")
...
if __name__ == "__main__":
    ...
    experiments = [       
        "airq_involuntary_switch_1_1/52_64_1_1_1_1_0_0_1_4",
    ]
    labels = [
        "Linux",
        "Linux + cIRQa",
    ...
    for index, experiment, n_thread in zip(range(len(experiments)), experiments, n_threads):
        ...
        # fig 1: x: #packets handled in softirq, y: nivcsw
        x_client = [thread.client_softirq_packets/1e6 for thread in threads_info if thread.client_core == core_selected[index]]
        y_client = [thread.client_nivcsw/1e6 for thread in threads_info if thread.client_core == core_selected[index]]
        plt.figure(figsize=(6,6*0.618))
        plt.scatter(x_client, y_client, color=colors[index], marker=markers[index], s=50, label=f"Client - {labels[index]}")
        plt.xlabel("#millions of packets processed")
        plt.ylabel("#millions of involuntary context switches")
        plt.grid(True, which="both", linestyle="--", linewidth=0.5)
        plt.tight_layout()
        plt.savefig(f"invcs_client_0_{core_selected[index]}_2.pdf")
        plt.close()

        # fig 2: x: nivcsw, y: vruntime increase
        x_client = [thread.client_nivcsw/1e6 for thread in threads_info if thread.client_core == core_selected[index]]
        # y_client = [thread.client_abs_vruntime for thread in threads_info if thread.client_core == core_selected[index]]
        y_client = norm_vruntime([thread.client_vruntime[-1] for thread in threads_info if thread.client_core == core_selected[index]])
        plt.figure(figsize=(6,6*0.618))
        plt.scatter(x_client, y_client, color=colors[index], marker=markers[index], s=50, label=f"Client - {labels[index]}")
        plt.xlabel("#millions of involuntary context switches")
        plt.ylabel("Virtual Runtime Increase (ns)")
        plt.grid(True, which="both", linestyle="--", linewidth=0.5)
        plt.tight_layout()
        plt.savefig(f"invcs_client_1_{core_selected[index]}_2.pdf")
        plt.close()

        # fig 3: x: vruntime increase, y: sched latency p99.9
        # x_client = [thread.client_abs_vruntime for thread in threads_info if thread.client_core == core_selected[index]]
        x_client = norm_vruntime([thread.client_vruntime[-1] for thread in threads_info if thread.client_core == core_selected[index]])
        y_client = [thread.client_sched_latency/1e3 for thread in threads_info if thread.client_core == core_selected[index]]
        plt.figure(figsize=(6,6*0.618))
        plt.scatter(x_client, y_client, color=colors[index], marker=markers[index], s=50, label=f"Client - {labels[index]}")
        plt.xlabel("Virtual Runtime Increase (ns)")
        plt.ylabel("P99.8 rx_sched Latency (us)")
        plt.grid(True, which="both", linestyle="--", linewidth=0.5)
        plt.tight_layout()
        plt.savefig(f"invcs_client_2_{core_selected[index]}_2.pdf")
        plt.close()

        # fig 4: x: nivcsw, y: sched latency p99.9
        x_client = [thread.client_nivcsw/1e6 for thread in threads_info if thread.client_core == core_selected[index]]
        y_client = [thread.client_sched_latency/1e3 for thread in threads_info if thread.client_core == core_selected[index]]
        plt.figure(figsize=(6,6*0.618))
        plt.scatter(x_client, y_client, color=colors[index], marker=markers[index], s=50, label=f"Client - {labels[index]}")
        ...
        plt.grid(True, which="both", linestyle="--", linewidth=0.5)
        plt.tight_layout()
        plt.savefig(f"invcs_client_3_{core_selected[index]}_2.pdf")
        plt.close()

        # fig 5: x: #packets handled in softirq, y: sched latency p99.9
        x_client = [thread.client_softirq_packets/1e6 for thread in threads_info if thread.client_core == core_selected[index]]
        y_client = [thread.client_sched_latency/1e3 for thread in threads_info if thread.client_core == core_selected[index]]
        plt.figure(figsize=(6,6*0.618))
        plt.scatter(x_client, y_client, color=colors[index], marker=markers[index], s=50, label=f"Client - {labels[index]}")
        ...
        plt.grid(True, which="both", linestyle="--", linewidth=0.5)
        plt.tight_layout()
        plt.savefig(f"invcs_client_4_{core_selected[index]}_2.pdf")
        plt.close()

        # fig 6-10: repeat for server side
        x_server = [thread.server_softirq_packets/1e6 for thread in threads_info if thread.server_core == core_selected[index]]
        y_server = [thread.server_nivcsw/1e6 for thread in threads_info if thread.server_core == core_selected[index]]
        plt.figure(figsize=(6,6*0.618))
        plt.scatter(x_server, y_server, color=colors[index], marker=markers[index], s=50, label=f"Server - {labels[index]}")
        plt.xlabel("#millions of packets processed")
        plt.ylabel("#millions of involuntary context switches")
        plt.grid(True, which="both", linestyle="--", linewidth=0.5)
        plt.tight_layout()
        plt.savefig(f"invcs_server_0_{core_selected[index]}_2.pdf")
        plt.close()

        x_server = [thread.server_nivcsw/1e6 for thread in threads_info if thread.server_core == core_selected[index]]
        # y_server = [thread.server_abs_vruntime for thread in threads_info if thread.server_core == core_selected[index]]
        y_server = norm_vruntime([thread.server_vruntime[-1] for thread in threads_info if thread.server_core == core_selected[index]])
        plt.figure(figsize=(6,6*0.618))
        plt.scatter(x_server, y_server, color=colors[index], marker=markers[index], s=50, label=f"Server - {labels[index]}")
        plt.xlabel("#millions of involuntary context switches")
        plt.ylabel("Virtual Runtime Increase (ns)")
        plt.grid(True, which="both", linestyle="--", linewidth=0.5)
        plt.tight_layout()
        plt.savefig(f"invcs_server_1_{core_selected[index]}_2.pdf")
        plt.close()

        # x_server = [thread.server_abs_vruntime for thread in threads_info if thread.server_core == core_selected[index]]
        x_server = norm_vruntime([thread.server_vruntime[-1] for thread in threads_info if thread.server_core == core_selected[index]])
        y_server = [thread.server_sched_latency/1e3 for thread in threads_info if thread.server_core == core_selected[index]]
        plt.figure(figsize=(6,6*0.618))
        plt.scatter(x_server, y_server, color=colors[index], marker=markers[index], s=50, label=f"Server - {labels[index]}")
        plt.xlabel("Virtual Runtime Increase (ns)")
        plt.ylabel("P99.8 rx_sched Latency (us)")
        plt.grid(True, which="both", linestyle="--", linewidth=0.5)
        plt.tight_layout()
        plt.savefig(f"invcs_server_2_{core_selected[index]}_2.pdf")
        plt.close()
        ...
        plt.grid(True, which="both", linestyle="--", linewidth=0.5)
        plt.tight_layout()
        plt.savefig(f"invcs_server_3_{core_selected[index]}_2.pdf")
        plt.close()
        ...
        plt.grid(True, which="both", linestyle="--", linewidth=0.5)
        plt.tight_layout()
        plt.savefig(f"invcs_server_4_{core_selected[index]}_2.pdf")
        plt.close()