...
def server_sched_parser(breakdown_log_path):
    ...
    for port, port_samples in samples.items():
        ...
        for idx, ts in enumerate(port_samples):
            if is_valid(ts['valid'], stage_bits['rx_data_copy']) and ts['rx_sleep_enter'] > ts['last_xmit_finish']:
            ...
            if is_valid(ts['valid'], stage_bits['app_hidden']) and ts['rx_sleep_enter'] > ts['last_xmit_finish']:
