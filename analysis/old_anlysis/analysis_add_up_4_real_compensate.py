...
def parse_all_thread_info(experiment_dir, total_threads):
    ...
    client_inflation_path = os.path.join(experiment_dir, f"valid_real_breakdown_sum_p90_client_compensate_p{comp}.txt")
    ...
    server_inflation_path = os.path.join(experiment_dir, f"valid_real_breakdown_sum_p90_server_compensate_p{comp}.txt")
    ...
    with open(client_inflation_path, "r") as client_inflation_log:
        ...
        for line in lines[1:]:
            items = line.split()
            port, total_samples = int(items[0]), float(items[-2])
            for thread in threads_info:
                if thread.port == port:
...
if __name__ == "__main__":
    ...
    n_threads = [52, 52, 52, 52, 52, 48, 48, 48, 48, 48]

    experiments = [
        f"sirq_understanding_{draw_serie}/52_64_1_1_1_1_0_0_1_0",
        f"sirq_understanding_{draw_serie}/52_64_1_1_1_1_0_0_1_1",
        f"sirq_understanding_{draw_serie}/52_64_1_1_1_1_0_0_1_2",
        f"sirq_understanding_{draw_serie}/52_64_1_1_1_1_0_0_1_3",
        f"sirq_understanding_{draw_serie}/52_64_1_1_1_1_0_0_1_4",
        # f"sirq_understanding_{draw_serie}/48_64_1_1_1_1_0_0_1_0",
        # f"sirq_understanding_{draw_serie}/48_64_1_1_1_1_0_0_1_1",
        # f"sirq_understanding_{draw_serie}/48_64_1_1_1_1_0_0_1_2",
        # f"sirq_understanding_{draw_serie}/48_64_1_1_1_1_0_0_1_3",
        # f"sirq_understanding_{draw_serie}/48_64_1_1_1_1_0_0_1_4",
    ]


    labels = [
        f"{draw_serie}_52_0_comp_{comp}",
        f"{draw_serie}_52_1_comp_{comp}",
        f"{draw_serie}_52_2_comp_{comp}",
        f"{draw_serie}_52_3_comp_{comp}",
        f"{draw_serie}_52_4_comp_{comp}",
        # f"{draw_serie}_48_0_comp_{comp}",
        # f"{draw_serie}_48_1_comp_{comp}",
        # f"{draw_serie}_48_2_comp_{comp}",
        # f"{draw_serie}_48_3_comp_{comp}",
        # f"{draw_serie}_48_4_comp_{comp}",
    ]
    ...
    for index, experiment, n_thread, label in zip(range(len(experiments)), experiments, n_threads, labels):
        ...
        # # [Option 1] Use the fixed k value according to nice value.
        # best_k = 300 / (1277/1024)
        best_k = 1 #/86

        # # [Option 2] Find the k to align the max throughput*inflation*k with abs_vruntime
        # rates = [i * j / k for i, j, k in zip(client_thpts, client_inflations, client_abs_vruntimes)]
        # max_rate = max([r for r in rates if r > 0])
        # best_k = 1 / max_rate

        # # [Option 3] Find the best k to minimize the MSE between client_abs_vruntimes and combined_metrics
        # best_k = 1.0
        # min_mse = float('inf')
        ...
        bar_width = 0.3
        ax_vruntime_bar.bar(np.array(thread_indices) - bar_width/2, client_abs_vruntimes, width=bar_width, label="Absolute virtual runtime", edgecolor="forestgreen", hatch='\\\\\\', fill=False, linewidth=2)
        ax_vruntime_bar.bar(np.array(thread_indices) + bar_width/2, combined_metrics, width=bar_width, label=f"Modeled runtime", edgecolor="firebrick", hatch='///', fill=False, linewidth=2)
        ax_vruntime_bar.set_xlabel("Thread Index")
        ax_vruntime_bar.set_ylabel("Absolute vruntime")
        ...
        # write the residual value and value/abs_vruntime ratio above each vruntime bar
        for i in range(len(thread_indices)):
            ax_vruntime_bar.text(thread_indices[i] - bar_width/2, max(client_abs_vruntimes + combined_metrics)*1.01, f"{client_residuals[i]/1e8:.1f}e8", ha='center', va='bottom', fontsize=6)
            ax_vruntime_bar.text(thread_indices[i] - bar_width/2, max(client_abs_vruntimes + combined_metrics)*1.02, f"{(client_residuals[i]/client_abs_vruntimes[i]*100):.1f}%", ha='center', va='bottom', fontsize=6)
        ...
        client_normed_start_vruntimes = norm_vruntime(client_start_vruntimes, 0)
        ...
        ax_relative_vruntime_bar.bar(np.array(thread_indices) + bar_width/2, client_normed_combined_metrics, width=bar_width, label=f"Modeled processing time", edgecolor="firebrick", hatch='///', fill=False, linewidth=2)
        ...
        # # best_k according to nice value
        # best_k = 300 / (1277/1024)
        best_k = 1 #/86

        # # best_k to align the max throughput*inflation*k with abs_vruntime
        # rates = [i * j / k for i, j, k in zip(server_thpts, server_inflations, server_abs_vruntimes)]
        # max_rate = max([r for r in rates if r > 0])
        # best_k = 1 / max_rate

        # # # best_k to minimize MSE
        # best_k = 1.0
        # min_mse = float('inf')
        # for k in np.arange(0.1, 10.0, 0.01):
        ...
        for i in range(len(thread_indices)):
            ...
            ax_vruntime_bar.text(thread_indices[i] - bar_width/2, max(server_abs_vruntimes + combined_metrics)*1.01, f"{residual/1e8:.1f}e8", ha='center', va='bottom', fontsize=6)
