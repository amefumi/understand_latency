...
experiments = ["sirq_eevdf_612_cores_1_1"]
saved_file_name = "latency_throughput_cores_dimon"
...
def parse_experiment(experiment_name):
    ...
    for n, f, i, d, p, perm, h, s, core in combinations:
        ...
        for run in runs:
            ...
            total_threads = n * core
            for j in range(total_threads):
                latency_bin_path = os.path.join(experiment_dir, f"netperf-{j}_hist.bin")
                latency_bins.append(read_histogram(latency_bin_path))

                throughput_log_path = os.path.join(experiment_dir, f"netperf-{j}_thpt.log") # This is not a fault though
                with open(throughput_log_path, "r") as throughput_log:
                    throughput += float(throughput_log.readlines()[0].split()[-1])
        ...
        data_index += 1
        print(f"Experiment {experiment_name}, n={n}, latency p999: {latency999}, throughput: {throughput/16}")
