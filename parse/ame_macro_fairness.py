...
def main():
        ...
        for run in runs:
            ...
            for i_thread in range(n_apps):
                ...
                with open(thread_netperf_log_path, "r") as thread_netperf_log:
                    ...
                    params = line.split()
                    run_throughput.append(float(params[5])/1e6)
            comb_throughput.append(tuple([run, run_avg_throughput] + run_throughput))
        
        if d == 1:
            combination_results_dim[num_apps.index(n)] += comb_throughput
        else:
            combination_results_dimoff[num_apps.index(n)] += comb_throughput
    ...
    for i in range(len(num_apps)):
        for j in range(len(combination_results_dim[i])):
            ...
            plt.scatter([ x ] * (len(combination_results_dim[i][j]) - 2), combination_results_dim[i][j][2:], color=colors[j], s=1)
    ...
    plt.ylabel('Throughput per run (mIOPS)')
