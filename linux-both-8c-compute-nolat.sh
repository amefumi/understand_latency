...
sudo sysctl -w net.core.latency_perstage_rdpmc_on=1 # enable rdpmc for latency breakdown
...
ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S sysctl -w net.core.latency_breakdown_validation=1"
...
ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S sysctl -w net.core.latency_perstage_rdpmc_on=1"
...
sudo zsh -c 'ulimit -n 65536; exec taskset -c '"$TASKSET"' nice -n -20 ./netdriver_test_multithread_nolat '"$TARGET"':'"$DPORT"' --count '"$N"'  --iodepth '"$IODEPTH"' --flowsize '"$SIZE"' --pin '"$PIN"' --sc '"$SC"' --time '"$CLIENT_TIME"' tcpppasync > temp/client.log' &

echo "echo $SUDOPW | sudo -S taskset -c $TASKSET nice -n -20 ./netdriver_test_multithread_nolat $TARGET:$DPORT --count $N  --iodepth $IODEPTH --flowsize $SIZE --pin $PIN --sc $SC --time $CLIENT_TIME tcpppasync"
PIDS="$PIDS $!"
...
./parse/parse-netperf-nolat.py $DIR $N > $DIR/linux_latency_nolat
