	...
	*port_name = 0;
	...
	for (nextArg = 2; (nextArg < argc) && (*argv[nextArg] == '-');
		...
		if (strcmp(argv[nextArg], "--help") == 0) {
			...
			// we now use sc to represent the CPU index directly
			// each thread only clone threads with this core index
			if (nextArg == (argc-1)) {
				printf("No value provided for %s option\n",
					argv[nextArg]);
				exit(1);
	...
	for(i = starting_index;  i < starting_index + thread_count; i++) {
		if (strcmp(argv[nextArg], "tcpppasync") == 0) {
			...
			std::cout << "launch thread " << i << " on core " << sc << "source port " << srcPort + i << std::endl;
			// void test_ndping_send(struct sockaddr *dest, int id, int io_depth, int flow_size, int src_port)
			if(pin == 1) {
				cpu_set_t cpuset;
				CPU_ZERO(&cpuset);
				CPU_SET(sc, &cpuset); // now we directly use sc as cpu index
