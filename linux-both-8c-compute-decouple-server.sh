...
echo "sudo taskset -c $TASKSET nice -n -20 $TARGETDIR/latency/pingpong_server  --ip $TARGET --port $((DPORT)) --count $N --iodepth $IODEPTH --flowsize $SIZE --pin $PIN --permute $PERMUTE --sc $SC > $TARGETDIR/latency/temp/server.log"
sleep 3

sudo taskset -c $TASKSET nice -n -20  ./netdriver_test_multithread $TARGET:$DPORT --count $N  --iodepth $IODEPTH --flowsize $SIZE --pin $PIN --sc 1 --time $CLIENT_TIME tcpppasync  > temp/client.log &
echo "sudo taskset -c $TASKSET nice -n -20  ./netdriver_test_multithread $TARGET:$DPORT --count $N  --iodepth $IODEPTH --flowsize $SIZE --pin $PIN --sc 1 --time $CLIENT_TIME tcpppasync"
PIDS="$PIDS $!"
