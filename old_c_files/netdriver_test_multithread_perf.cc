...
std::vector<ThreadPerf> g_perf_events;

static ThreadPerf open_perfs_for_this_thread() {
	ThreadPerf tp;
	tp.tid = syscall(__NR_gettid);
	for (int i = 0; i < PERF_EVENT_COUNT; ++i) {
		tp.fds[i] = -1;
	}
	...
	// Open first event in group: CACHE_REFERENCES
	attr.config = PERF_COUNT_HW_CACHE_REFERENCES;
	int cache_ref_fd = syscall(__NR_perf_event_open, &attr, tp.tid, -1, -1, 0);
	if (cache_ref_fd < 0) {
		perror("perf_event_open CACHE_REFERENCES");
		exit(EXIT_FAILURE);
	}
	tp.fds[CACHE_REFERENCES] = cache_ref_fd;

	// Open second event in group: CACHE_MISSES
	attr.config = PERF_COUNT_HW_CACHE_MISSES;
	int cache_miss_fd = syscall(__NR_perf_event_open, &attr, tp.tid, -1, cache_ref_fd, 0);
	if (cache_miss_fd < 0) {
		perror("perf_event_open CACHE_MISSES");
		exit(EXIT_FAILURE);
	}
	tp.fds[CACHE_MISSES] = cache_miss_fd;

	return tp;

}

void dump_thread_perf(const ThreadPerf &tp) {
	int leading_fd = tp.fds[CACHE_REFERENCES];
	if (leading_fd < 0) {
		return;
	}
	uint64_t values[PERF_EVENT_COUNT + 1];
	ssize_t ret = read(leading_fd, &values, sizeof(values));
	if (ret < 0) {
		perror("read perf event");
		exit(EXIT_FAILURE);
	}
	uint64_t nr = values[0];
	printf("tid: %d events: %lu ", tp.tid, nr);
	for (uint64_t i = 0; i < nr; ++i) {
		printf("event-%lu: %lu ", (unsigned long) i, (unsigned long) values[i + 1]);
	}
	printf("\n");
}

void dump_all_thread_perfs() {
	for (ThreadPerf &tp : g_perf_events) {
		dump_thread_perf(tp);
	}
}

void dump_after_second(int seconds) {
	std::this_thread::sleep_for(std::chrono::seconds(seconds));
	dump_all_thread_perfs();
	fflush(stdout);
}
/* Perf-related End */
...
//	sched_setscheduler(pid, SCHED_RR, &param);
	...
	ThreadPerf tp = open_perfs_for_this_thread();
	{
		std::lock_guard<std::mutex> lock(g_perf_mutex);
		g_perf_events.push_back(tp);
	}
