...
#LOG=249
# Tianyu: temporarily set from 200 to 500 to reduce log size for multiple core scenes.
...
then
	...
	# echo 999999999 | sudo tee /proc/sys/kernel/sched_latency_ns  # Note: this is used to disable vruntime clamp
	...
	# ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S -v; echo 999999999 | sudo tee /proc/sys/kernel/sched_latency_ns" # Note: this is used to disable vruntime clamp
...
#	sudo ethtool -C $INTF rx-frames $PKT rx-usecs $TIMEOUT tx-frames $PKT tx-usecs $TIMEOUT
    ...
    USECS=$(((11*N/SC+7)/8)) # usecs = (N/4) * (1/(0.4/2)) = 1.25 * N
    FRAMES=$(( FRAMES > 8 ? FRAMES : 8 ))
    USECS=$(( USECS  > 32 ? USECS  : 32  ))
    # FRAMES=64
    # USECS=64
i
    echo "Automatic DIM tuning:"
    echo "rx-frames: $FRAMES"
    echo "rx-usecs:  $USECS"
...
SC=$((SC * 2)) # Note: this is only enabled in "core" shell script, but as we reuse this script for single physical core situation, we need to keep this line.
...
# sudo insmod $TARGETDIR/iter_thread/iter_thread.ko total_count=3000 interval_ms=100
# ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S insmod $TARGETDIR/iter_thread/iter_thread.ko total_count=3000 interval_ms=100"
...
ssh "$USER@$TARGETC" -tt -- \
  "echo '$SUDOPW' | sudo -S zsh $TARGETDIR/latency/start_server.sh \
     '$TASKSET' \
     '$TARGETDIR' \
     '$TARGET' \
     '$DPORT' \
...
IFS=',' read -r -a CORES <<< "$TASKSET"

for i in "${!CORES[@]}"; do
    core="${CORES[$i]}"
    port=$((DPORT + i))
	threads=$((N / SC))
    echo "Launching thread $i on core $core, port $port"
    sudo zsh -c "
        ulimit -n 65536
        exec taskset -c $core nice -n -20 \
        ./netdriver_test_multithread_cores ${TARGET}:${port} \
            --count ${threads} \
            --iodepth ${IODEPTH} \
            --flowsize ${SIZE} \
            --pin ${PIN} \
...
# sudo sysctl -w net.core.latency_breakdown_validation=0
# sudo sysctl -w net.core.latency_dumb_schedule_enable=0
# sudo sysctl -w net.core.latency_dumb_schedule_weight=0
# sudo sysctl -w net.core.latency_dumb_schedule_disable_clamp=1
...
# ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S sysctl -w net.core.latency_breakdown_validation=0"
# ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S sysctl -w net.core.latency_dumb_schedule_enable=0"
# ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S sysctl -w net.core.latency_dumb_schedule_weight=0"
# ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S sysctl -w net.core.latency_dumb_schedule_disable_clamp=1"
...
# sudo tail -n 8000  /var/log/kern.log > $DIR/iter_thread_client.log
# ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S rmmod iter_thread"
# ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S tail -n 8000  /var/log/kern.log" > $DIR/iter_thread_server.log
