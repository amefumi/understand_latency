...
#	sudo ethtool -C $INTF rx-frames $PKT rx-usecs $TIMEOUT tx-frames $PKT tx-usecs $TIMEOUT
	...
	echo "half dim: rx off, tx on"
	ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S ethtool -C $INTF adaptive-rx off adaptive-tx on"
	sudo ethtool -C $INTF adaptive-rx off adaptive-tx on
fi
