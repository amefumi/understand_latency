...
#	sudo ethtool -C $INTF rx-frames $PKT rx-usecs $TIMEOUT tx-frames $PKT tx-usecs $TIMEOUT
	...
	# echo "server dim on, client dim off"
	# ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S ethtool -C $INTF adaptive-rx on adaptive-tx on"
	# sudo ethtool -C $INTF adaptive-rx off adaptive-tx off
	echo "server dim off, client dim off"
	ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S ethtool -C $INTF adaptive-rx off adaptive-tx off"
	sudo ethtool -C $INTF adaptive-rx off adaptive-tx off
	ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S -v && sudo ethtool -C $INTF rx-frames 64 rx-usecs 32"
fi
...
# enable iter thread (vruntime)
# sudo insmod $TARGETDIR/iter_thread/iter_thread.ko
# ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S insmod $TARGETDIR/iter_thread/iter_thread.ko"
...
echo "sudo taskset -c $TASKSET nice -n -20 $TARGETDIR/latency/pingpong_server_rr  --ip $TARGET --port $((DPORT)) --count $N --iodepth $IODEPTH --flowsize $SIZE --pin $PIN --permute $PERMUTE --sc $SC > $TARGETDIR/latency/temp/server.log"
sleep 3

sudo taskset -c $TASKSET nice -n -20  ./netdriver_test_multithread_rr $TARGET:$DPORT --count $N  --iodepth $IODEPTH --flowsize $SIZE --pin $PIN --sc 1 --time $CLIENT_TIME tcpppasync  > temp/client.log &
echo "sudo taskset -c $TASKSET nice -n -20  ./netdriver_test_multithread_rr $TARGET:$DPORT --count $N  --iodepth $IODEPTH --flowsize $SIZE --pin $PIN --sc 1 --time $CLIENT_TIME tcpppasync"
PIDS="$PIDS $!"
...
ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S trace-cmd clear"
ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S killall pingpong_server_nh"
ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S -v && echo 0 | sudo tee /sys/module/core/parameters/accu_irq_accounting"
...
# sudo tail -n 230  /var/log/kern.log > temp/filter_client.log
# ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S -v && sudo tail -n 230  /var/log/kern.log" > temp/filter_server.log
...
# remove iter_thread
# 550 for only iter_thread, 700 for both iter_thread and filter
# sudo tail -n 700  /var/log/kern.log > $DIR/iter_thread_client.log
# sudo rmmod iter_thread
# ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S tail -n 700  /var/log/kern.log" > $DIR/iter_thread_server.log
