...
ssh $USER\@$TARGETC -t "echo $SUDOPW | sudo -S taskset -c $TASKSET nice -n -20 $TARGETDIR/latency/pingpong_server  --ip $TARGET --port $((DPORT)) --count $N --iodepth $IODEPTH --flowsize $SIZE --pin $PIN --permute $PERMUTE --sc 1 > $TARGETDIR/latency/temp/server.log" &
echo "sudo taskset -c $TASKSET nice -n -20 $TARGETDIR/latency/pingpong_server  --ip $TARGET --port $((DPORT)) --count $N --iodepth $IODEPTH --flowsize $SIZE --pin $PIN --permute $PERMUTE --sc 1 > $TARGETDIR/latency/temp/server.log"
sleep 3
...
echo "sudo taskset -c $TASKSET nice -n -20  ./netdriver_test_multithread $TARGET:$DPORT --count $N  --iodepth $IODEPTH --flowsize $SIZE --pin $PIN --sc $SC --time $CLIENT_TIME tcpppasync"
PIDS="$PIDS $!"
