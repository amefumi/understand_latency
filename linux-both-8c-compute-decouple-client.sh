...
then
	...
	# Note: we always disable client side DIM in incast experiment: client is not the bottleneck, we need it react quickly.
else
...
ssh $USER\@$TARGETC -t "
  echo '$SUDOPW' | sudo -S zsh -c '
    ulimit -n 65536
    exec taskset -c $TASKSET_SERVER nice -n -20 $TARGETDIR/latency/pingpong_server \
...
echo "sudo taskset -c $TASKSET_SERVER nice -n -20 $TARGETDIR/latency/pingpong_server  --ip $TARGET --port $((DPORT)) --count $N --iodepth $IODEPTH --flowsize $SIZE --pin $PIN  > $TARGETDIR/latency/temp/server.log"
sleep 3
